...
import { getCurrentDay, getCurrentMonth } from '../../redux/water/waterSelectors';
  ...
  useEffect(() => {
    ...
    if (compare === 0) {
        if (currentMonthStatistic && currentDayStatistic) {
          const updatedData = updateOrAddCurrentDay(currentMonthStatistic, currentDayStatistic);
...
if (compare === -1) {
  ...
  }, [selectedMonth, currentMonthStatistic, currentDayStatistic, setMonthStatistic]);

  return (
    <div>
      <MonthSwitcher selectedMonth = {selectedMonth} setSelectedMonth={setSelectedMonth}/>
      <MonthStatistic selectedMonth={selectedMonth} monthStatistic={monthStatistic} setSelectedMonth={setSelectedMonth } /> 
    </div>
  );
};
